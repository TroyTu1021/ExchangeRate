		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("DocEntry") === oObject.DocEntry;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].DocEntry;
									} else {
										sCurrentId = aItemsNotInTheList[0].DocEntry;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("DocEntry"),
						title: oBindingContext.getProperty("CardName")
					};
				}
			},
			...
			assertions: {
				...
				theListShouldBeSortedAscendingOnName: function() {
					return this.theListShouldBeSortedAscendingOnField("CardName");
				},
